...
export const analyzeDiagram = async ({ nodes, edges, mode, apiKey, modelId }) => {
  if (!apiKey) throw new Error('Google API 키를 입력해주세요.');
  if (nodes.length === 0) throw new Error('분석할 구성원이 없습니다. 먼저 가계도/생태도를 작성해주세요.');
